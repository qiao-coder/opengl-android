 ...
 * @date 2022/4/12.
 */
public class BasicLightingDiffuse extends GLSurfaceView {
    private static final String TAG = "LightSample";

    private BasicLightingDiffuseRender basicLightingDiffuseRender;
    ...
    public BasicLightingDiffuse(Context context, AttributeSet attrs) {
        ...
        basicLightingDiffuseRender = new BasicLightingDiffuseRender(context);
        setRenderer(basicLightingDiffuseRender);
        setRenderMode(RENDERMODE_WHEN_DIRTY);
        setFocusable(true);
    }

    public void move(Direction d) {
        basicLightingDiffuseRender.move(d, 0.05f);
        requestRender();
    }

    public void move(Direction d, float cameraSpeed) {
        basicLightingDiffuseRender.move(d, cameraSpeed);
        requestRender();
    }

    public void setYaw(float yaw) {
        basicLightingDiffuseRender.setYaw(yaw);
        requestRender();
    }

    public void setPitch(float pitch) {
        basicLightingDiffuseRender.setPitch(pitch);
        requestRender();
    }
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (event.getPointerCount() == 2) {
                    ...
                    basicLightingDiffuseRender.setFov(fov);
                    requestRender();
                }
                break;
            }
