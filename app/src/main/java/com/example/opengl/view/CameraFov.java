...
import com.example.opengl.render.CameraFovRender;

/**
 * @author wuzhanqiao
 * @date 2022/6/16.
 */
public class CameraFov extends GLSurfaceView {
    private static final String TAG = "CameraFov";

    private CameraFovRender cameraFovRender;
    private float lastDist = 0.0f;
    private final static float minFov = 1.0f;
    private final static float maxFov = 45.0f;
    private float fov = 45.0f;

    public CameraFov(Context context) {
        this(context, null);
    }

    public CameraFov(Context context, AttributeSet attrs) {
        super(context, attrs);
        setEGLContextClientVersion(3);
        cameraFovRender = new CameraFovRender(context);
        setRenderer(cameraFovRender);
        setRenderMode(RENDERMODE_WHEN_DIRTY);
        setFocusable(true);
    }

    public void move(Direction d) {
        cameraFovRender.move(d, 0.05f);
        requestRender();
    }

    public void move(Direction d, float cameraSpeed) {
        cameraFovRender.move(d, cameraSpeed);
        requestRender();
    }

    public void setYaw(float yaw) {
        cameraFovRender.setYaw(yaw);
        requestRender();
    }

    public void setPitch(float pitch) {
        cameraFovRender.setPitch(pitch);
        requestRender();
    }
